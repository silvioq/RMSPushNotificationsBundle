...
class AndroidFCMNotification implements OSNotificationServiceInterface
{
    ...
    /**
     * Whether or not to use the dry run FCM
     *
     * @var bool
     */
    ...
    public function send(MessageInterface $message)
    {
        if (!$message instanceof AndroidMessage) {
            throw new InvalidMessageTypeException(
                sprintf(
                    "Message type '%s' not supported by FCM",
                    get_class(
                        $message
                    )
                )
        ...
        $data    = array_merge(
            ...
            array("notification" => ["body" => $message->getMessage()])
        );
        ...
        $gcmIdentifiers  = $message->getFCMIdentifiers();
